...
export default function ChargesDataTable(props: {
  data?: SavingsAccountCharge[];
  setChargesData: React.Dispatch<React.SetStateAction<SavingsAccountCharge[]>>;
  ...
  const onDelete = (id: number) => {
    ...
    // Ensure formValues.SavingsAccountCharges is updated
    setFormValues((prevFormValues) => ({
      ...prevFormValues,
      savingsAccountCharges: updatedData.length > 0 ? updatedData : [],
    }));
  };

  const onAmountChange = (
    value: number | null,
    record: SavingsAccountCharge
  ) => {
    ...
    setFormValues((prevFormValues) => ({
      ...prevFormValues,
      savingsAccountCharges: updatedData, // Ensure the updated charges persist
    }));
  };

  const columns: TableProps<SavingsAccountCharge>["columns"] = [
    {
      title: "Name",
      dataIndex: "name",
      key: "name",
