...
export default function DetailsForm(props: {
  ...
  setSelectedDepositProduct: React.Dispatch<
    React.SetStateAction<Partial<DepositProductTermAndPreClosure>>
  >;
  showFixedDepositProduct: boolean;
  setShowFixedDepositProduct: React.Dispatch<React.SetStateAction<boolean>>;
}) {
  ...
  useEffect(() => {
    if (submitType === "update") {
      setShowFixedDepositProduct(true);
    }
  }, [submitType]);
  ...
  const onFinish = (values: any) => {
    setFormValues({
      ...
      clientId: id,
      allowOverdraft:
        formValues.enforceMinRequiredBalance === true ? false : true,
      enforceMinRequiredBalance:
        formValues.allowOverdraft === true ? false : true,
      minRequiredBalance:
        formValues.allowOverdraft === true
          ? null
          : formValues.minRequiredBalance,
      lockInPeriodFrequencyEnum:
        selectedDepositProduct.lockInPeriodFrequencyTypeEnum,
      savingsProduct: selectedDepositProduct || formValues.savingsProduct,
      //charges: selectedDepositProduct.charges,
    });
  ...
  const {
    ...
    `${tenantId}/savings-products?filter={"where":{"depositTypeEnum":"FIXED DEPOSIT"}}`,
    [
      `${tenantId}/savings-products?filter={"where":{"depositTypeEnum":"FIXED DEPOSIT"}}`,
    ]
  );
  ...
  const onChangeSavingsProduct = (value: number) => {
    ...
    setShowFixedDepositProduct(false);
  };
  ...
  useEffect(() => {
    if (selectedSavingsProductId) {
      get(`${tenantId}/savings-products/${selectedSavingsProductId}`).then(
        (res) => {
          setShowFixedDepositProduct(true);
          setSelectedDepositProduct(res.data);
